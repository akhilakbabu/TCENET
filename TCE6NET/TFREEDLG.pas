...
begin
 ...
 UseDay:=TeachersFreeDay;
 If UseDay<0 then UseDay:=0;
 teachersfreeshow1:=winView[wnTeFree]+1;
 ...
 FillComboYears;
 listbox1.clear;
 for i:=1 to codeCount[1] do listbox1.items.add(tecode[codepoint[i,1],0]);
 listbox2.clear;
 {add saved selection here}
 rangeCheckCodeSels(TeFreeSelect,1);
 for i:=0 to nmbrteachers do tmpSel[i]:=TeFreeSelect[i];
 case teachersfreeshow1 of
  1: radiobutton1.checked:=true;
  2: radiobutton2.checked:=true;
  3: radiobutton3.checked:=true;
 end;
 case teachersfreeshow2 of
  1: radiobutton6.checked:=true;
  2: radiobutton7.checked:=true;
  3: begin
       rbYear.Checked := true;
       cmbYear.ItemIndex := teFreeYear+1;
     end;
 end;
 duringload:=false;
end;
...
begin
 if combobox1.enabled then
  if teachersfreeday=-2 then
  begin
   ComboMsg('Check day selected',ComboBox1);
   exit;
  end;
 if combobox3.enabled then
  if TeFreePeriod<0 then
  begin
   ComboMsg('Check time slot selected',ComboBox3);
   exit;
  end;
 for i:=0 to nmbrteachers do TeFreeSelect[i]:=tmpSel[i];
 if radiobutton1.checked then teachersfreeshow1:=1
  else if radiobutton2.checked then teachersfreeshow1:=2
   else if radiobutton3.checked then teachersfreeshow1:=3;
 if radiobutton6.checked then teachersfreeshow2:=1
  else if radiobutton7.checked then teachersfreeshow2:=2
  else if rbYear.Checked then
  begin
    teachersfreeshow2 := 3;
    teFreeYear := findyear(cmbYear.Text);
  end;
       
 winView[wnTeFree]:=teachersfreeshow1-1;
 close;
...
begin
 ...
 if tmpSel[0]>0 then
   for i:=1 to tmpSel[0] do
    listbox2.items.add(tecode[tmpSel[i],0]);

 label6.caption:=inttostr(listbox1.items.count);
 label7.caption:=inttostr(listbox2.items.count);
 if teachersfreeday=-1 then combobox1.itemindex:=0
   else combobox1.itemindex:=teachersfreeday+1;
 fillTimeSlots;

 if TeFreePeriod=-1 then combobox3.itemindex:=0
   else combobox3.itemindex:=TeFreePeriod;
 combobox2.itemindex:=teachersfreefac-1;
 checkRadiobuttons;
end;
...
begin
 if duringload then exit;
 olddayGroup:=DayGroup[teachersfreeday];
 teachersfreeday:=findday(combobox1.text);
 if teachersfreeday>=0
  then
   begin
    label12.caption:=day[teachersfreeday];
    UseDay:=TeachersFreeDay;
   end
 else
  begin
...
begin
 if duringload then exit;
 if trim(combobox2.text)='' then teachersfreefac:=-1  {none selected}
  else teachersfreefac:=findfaculty(ComboBox2.text,label12); {0 if not found and -1 for not selected}
end;
...
begin
 if duringload then exit;
 TeFreePeriod:=Combobox3.ItemIndex;
 if TeFreePeriod>0 then label8.caption:=TimeSlotName[UseDay,TeFreePeriod-1]
  else if TeFreePeriod=0 then label8.caption:='All time slots'
